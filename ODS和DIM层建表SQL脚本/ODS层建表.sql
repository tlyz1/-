...
CREATE TABLE IF NOT EXISTS ods.ods_mem_user_point_detailed_i(
    id           BIGINT     COMMENT '主键',
    zt_id        BIGINT    COMMENT '用户联合id',
    subject_id   BIGINT    COMMENT '关联主体id',
    point_t0     BIGINT    COMMENT '主体的t0积分',
    point_t1     BIGINT    COMMENT '主体的t1积分',
    frozen_point BIGINT    COMMENT '冻结积分',
    created_time TIMESTAMP  COMMENT '创建时间',
    updated_time TIMESTAMP  COMMENT '修改时间',
    is_deleted   BIGINT     COMMENT '是否删除0正常，1删除'
)
...
CREATE TABLE IF NOT EXISTS ods.ods_sale_store_target_f
(
    id                BIGINT COMMENT '主键',
    ...
    act_month         STRING COMMENT '实施月份',
    is_deleted        BIGINT COMMENT '是否删除0:未删除；1：已删除'
)
COMMENT '门店月目标销售额'
row format delimited fields terminated by ','
stored as orc
...
CREATE TABLE IF NOT EXISTS ods.ods_sale_city_category_target_f
(
    id                  BIGINT COMMENT '主键',
    ...
    act_month           STRING COMMENT '有效月份',
    sale_month_target   DECIMAL(27, 2) COMMENT '目标金额',
    create_user         STRING COMMENT '创建人',
    create_date         STRING COMMENT '创建时间',
    is_deleted          BIGINT COMMENT '0：正常，1：已删除'
)
COMMENT '城市分类销售目标表'
row format delimited fields terminated by ','
stored as orc
...
CREATE TABLE IF NOT EXISTS ods.ods_stock_store_goods_stock_data_day_i(
    id                  BIGINT COMMENT '主键',
